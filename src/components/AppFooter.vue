<template>
  <footer class="footer mt-auto py-4 bg-success">
    <div class="container-fluid text-light">
      <div class="row row-cols-1 row-cols-md-3 row-cols-lg-5 g-4">
        
        <!-- About Us -->
        <div class="col">
          <h5 class="fw-bold">About us</h5>
          <ul class="list-unstyled">
            <li><a href="#" class="text-light text-decoration-none">Courses</a></li>
            <li><a href="#" class="text-light text-decoration-none">Mission</a></li>
            <li><a href="#" class="text-light text-decoration-none">Approach</a></li>
            <li><a href="#" class="text-light text-decoration-none">Efficacy</a></li>
            <li><a href="#" class="text-light text-decoration-none">Handbook</a></li>
            <li><a href="#" class="text-light text-decoration-none">Research</a></li>
            <li><a href="#" class="text-light text-decoration-none">Careers</a></li>
            <li><a href="#" class="text-light text-decoration-none">Store</a></li>
            <li><a href="#" class="text-light text-decoration-none">Investors</a></li>
            <li><a href="#" class="text-light text-decoration-none">Contact us</a></li>
          </ul>
        </div>

        <!-- Products -->
        <div class="col">
          <h5 class="fw-bold">Products</h5>
          <ul class="list-unstyled">
            <li><a href="#" class="text-light text-decoration-none">Duolingo</a></li>
            <li><a href="#" class="text-light text-decoration-none">Duolingo for Schools</a></li>
            <li><a href="#" class="text-light text-decoration-none">English Test</a></li>
            <li><a href="#" class="text-light text-decoration-none">Duolingo Math</a></li>
            <li><a href="#" class="text-light text-decoration-none">Podcast</a></li>
          </ul>
        </div>

        <!-- Apps -->
        <div class="col">
          <h5 class="fw-bold">Apps</h5>
          <ul class="list-unstyled">
            <li><a href="#" class="text-light text-decoration-none">Android</a></li>
            <li><a href="#" class="text-light text-decoration-none">iOS</a></li>
          </ul>
        </div>

        <!-- Help and Support -->
        <div class="col">
          <h5 class="fw-bold">Help & Support</h5>
          <ul class="list-unstyled">
            <li><a href="#" class="text-light text-decoration-none">FAQs</a></li>
            <li><a href="#" class="text-light text-decoration-none">Schools FAQs</a></li>
            <li><a href="#" class="text-light text-decoration-none">Test FAQs</a></li>
            <li><a href="#" class="text-light text-decoration-none">Status</a></li>
          </ul>
        </div>

        <!-- Privacy & Social -->
        <div class="col">
          <h5 class="fw-bold">Privacy & Terms</h5>
          <ul class="list-unstyled">
            <li><a href="#" class="text-light text-decoration-none">Community Guidelines</a></li>
            <li><a href="#" class="text-light text-decoration-none">Terms</a></li>
            <li><a href="#" class="text-light text-decoration-none">Privacy</a></li>
          </ul>

          <h5 class="fw-bold mt-3">Follow Us</h5>
          <ul class="list-unstyled">
            <li><a href="#" class="text-light text-decoration-none">Instagram</a></li>
            <li><a href="#" class="text-light text-decoration-none">TikTok</a></li>
            <li><a href="#" class="text-light text-decoration-none">Twitter</a></li>
            <li><a href="#" class="text-light text-decoration-none">YouTube</a></li>
          </ul>
        </div>

      </div>
    </div>
  </footer>
</template>

<script>
export default {
  name: 'FooterComponent',
};
</script>

<style scoped>
/* Custom styles if needed */
</style>
